...
pub struct GodotM8Client {
    ...
    audio_backend: Option<audio::AudioBackend>,
...
impl GodotM8Client {
    ...
    fn audio_try_init(&mut self) {
        if self.audio_backend.is_none() {
            self.audio_backend = match audio::AudioBackend::new() {
    ...
    fn audio_list_input_devices(&mut self) -> Vec<GString> {
        ...
        let device_names = match &self.audio_backend {
            Some(audio_backend) => audio_backend.input_device_names().unwrap_or_default(),
            None => vec![],
        };
    ...
    fn set_volume(&mut self, volume: f32) {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            audio_backend.set_volume(volume);
        }
    }
    ...
    fn get_volume(&mut self) -> f32 {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            if let Some(volume) = audio_backend.volume() {
                return volume;
            }
        }
        0.0
    ...
    fn get_audio_peak_volume(&mut self) -> Vector2 {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            if let Some(peaks) = audio_backend.peaks() {
    ...
    fn get_audio_spec(&mut self) -> VarDictionary {
        ...
        if let Some(_audio_backend) = self.audio_backend.as_mut() {
            let _ = dict.insert("driver_name", "n/a");
            let _ = dict.insert("sample_rate", "n/a");
            let _ = dict.insert("buffer_size", "n/a");
            let _ = dict.insert("latency_ms", "n/a");
            let _ = dict.insert("num_channels", "n/a");
        }
        dict
    }
    ...
    pub fn get_audio_magnitude_at_freq(&mut self, freq: f32) -> f32 {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            if let Some(magnitude) = audio_backend.magnitude_at_freq(freq) {
                return magnitude;
            }
        }
        0.0
    ...
    pub fn is_spectrum_analyzer_enabled(&mut self) -> bool {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            return audio_backend.spectrum_analyzer_enabled();
        }
        false
    }
    ...
    pub fn set_spectrum_analyzer_enabled(&mut self, enabled: bool) {
        if let Some(audio_backend) = self.audio_backend.as_mut() {
            audio_backend.set_spectrum_analyzer_enabled(enabled);
        }
    }
}
