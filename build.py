...
def run(command: str, working_directory: str = None, env = None) -> int:
    ...
    if not using_cygwin():
        ...
        bash_path = find_bash()
        if bash_path:
            result = subprocess.run([bash_path, "--login", "-c", command], check=False, env=env)
...
if build_extension:
    ...
    for target in cargo_targets:
        if run("%s target add %s" % (rustup_path, target)) != 0:
            quit(1)
        if run("%s build --target %s" % (cargo_path, target), "libm8gd") != 0:
            quit(1)
        # output_path = Path("libm8gd/target/%s/" % target)
        # for file in list(itertools.chain(output_path.rglob("libm8gd.dll"), output_path.rglob("liblibm8gd.so"), output_path.rglob("liblibm8gd.dylib"))):
