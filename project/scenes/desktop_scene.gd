...
@export var surface_color := Color(1.0, 1.0, 1.0):
	set(value):
		...
		if surface_mesh.material_override:
			surface_mesh.material_override.albedo_color = value
...
@export var surface_enable_grid := false:
	set(value):
		...
		if value:
			...
			surface_mesh.material_overlay.albedo_color = surface_grid_color
		else:
...
@export var surface_grid_color := Color.WHITE:
	set(value):
		...
		if surface_mesh.material_overlay:
			surface_mesh.material_overlay.albedo_color = value
...
func set_surface_mode(index: int) -> void:
	...
	var material: StandardMaterial3D = surface_mesh.material_override
	match index:
		0:  # wood
